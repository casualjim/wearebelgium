...
object PluginsBuild extends Build {
  ...
  val jelasticDeploy = file("/Users/ivan/projects/sbt-jelastic-deploy")
//  val sbtRequireJs = uri("git://github.com/scalatra/sbt-requirejs")
//  val sbtRequireJs = file("/Users/ivan/projects/sbt-requirejs")
  lazy val root = Project("plugins", file(".")) //dependsOn(jelasticDeploy) //.dependsOn(startScript) //.dependsOn(sbtRequireJs) //settings (scalacOptions += "-deprecation")
